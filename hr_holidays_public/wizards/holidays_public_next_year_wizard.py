...
class HolidaysPublicNextYearWizard(models.TransientModel):
    ...
    warning_existing = fields.Boolean(
        compute="_compute_warning_existing",
        store=True,
    )
    ...
    def _onchange_template_id(self):
        ...
        vals_list = []
        for line in self.template_id.line_ids.filtered(lambda r: r.variable_date):
    ...
    def _compute_warning_existing(self):
        for record in self:
            existing = self.env["hr.holidays.public"].search(
                [("country_id", "=", record.country_id.id), ("year", "=", record.year)],
                limit=1,
            )
    ...
    def create_public_holidays(self):
        ...
        for line in self.template_id.line_ids.filtered(lambda r: not r.variable_date):
            date = fields.Date.from_string(line.date)
